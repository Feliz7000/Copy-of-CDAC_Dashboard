...
-- Purpose: Per-module (per-slot) breakdown for CCEE and IA, including totals and ranks
-- =============================================================================
...
WITH base AS (
    ...
    SELECT
        batch_name,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_01') AS cc_max_t01,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_02') AS cc_max_t02,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_03') AS cc_max_t03,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_04') AS cc_max_t04,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_05') AS cc_max_t05,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_06') AS cc_max_t06,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_07') AS cc_max_t07,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_08') AS cc_max_t08
    FROM test_mapping
    ...
    SELECT
        batch_name,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_01') AS ia_max_t01,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_02') AS ia_max_t02,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_03') AS ia_max_t03,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_04') AS ia_max_t04,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_05') AS ia_max_t05,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_06') AS ia_max_t06,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_07') AS ia_max_t07,
        MAX(max_marks) FILTER (WHERE is_active = TRUE AND column_slot = 'test_08') AS ia_max_t08
    FROM test_mapping
    WHERE category_code = 'IA'
    GROUP BY batch_name
), module_calc AS (
    SELECT
        b.*,
        CASE WHEN b.cc_t01 IS NOT NULL OR b.ia_t01 IS NOT NULL THEN COALESCE(b.cc_t01, 0) + COALESCE(b.ia_t01, 0) END AS module_01_total,
        CASE WHEN b.cc_t02 IS NOT NULL OR b.ia_t02 IS NOT NULL THEN COALESCE(b.cc_t02, 0) + COALESCE(b.ia_t02, 0) END AS module_02_total,
        CASE WHEN b.cc_t03 IS NOT NULL OR b.ia_t03 IS NOT NULL THEN COALESCE(b.cc_t03, 0) + COALESCE(b.ia_t03, 0) END AS module_03_total,
        CASE WHEN b.cc_t04 IS NOT NULL OR b.ia_t04 IS NOT NULL THEN COALESCE(b.cc_t04, 0) + COALESCE(b.ia_t04, 0) END AS module_04_total,
        CASE WHEN b.cc_t05 IS NOT NULL OR b.ia_t05 IS NOT NULL THEN COALESCE(b.cc_t05, 0) + COALESCE(b.ia_t05, 0) END AS module_05_total,
        CASE WHEN b.cc_t06 IS NOT NULL OR b.ia_t06 IS NOT NULL THEN COALESCE(b.cc_t06, 0) + COALESCE(b.ia_t06, 0) END AS module_06_total,
        CASE WHEN b.cc_t07 IS NOT NULL OR b.ia_t07 IS NOT NULL THEN COALESCE(b.cc_t07, 0) + COALESCE(b.ia_t07, 0) END AS module_07_total,
        CASE WHEN b.cc_t08 IS NOT NULL OR b.ia_t08 IS NOT NULL THEN COALESCE(b.cc_t08, 0) + COALESCE(b.ia_t08, 0) END AS module_08_total,
        CASE WHEN cm.cc_max_t01 IS NOT NULL OR im.ia_max_t01 IS NOT NULL THEN COALESCE(cm.cc_max_t01, 0) + COALESCE(im.ia_max_t01, 0) END AS module_01_max,
        CASE WHEN cm.cc_max_t02 IS NOT NULL OR im.ia_max_t02 IS NOT NULL THEN COALESCE(cm.cc_max_t02, 0) + COALESCE(im.ia_max_t02, 0) END AS module_02_max,
        CASE WHEN cm.cc_max_t03 IS NOT NULL OR im.ia_max_t03 IS NOT NULL THEN COALESCE(cm.cc_max_t03, 0) + COALESCE(im.ia_max_t03, 0) END AS module_03_max,
        CASE WHEN cm.cc_max_t04 IS NOT NULL OR im.ia_max_t04 IS NOT NULL THEN COALESCE(cm.cc_max_t04, 0) + COALESCE(im.ia_max_t04, 0) END AS module_04_max,
        CASE WHEN cm.cc_max_t05 IS NOT NULL OR im.ia_max_t05 IS NOT NULL THEN COALESCE(cm.cc_max_t05, 0) + COALESCE(im.ia_max_t05, 0) END AS module_05_max,
        CASE WHEN cm.cc_max_t06 IS NOT NULL OR im.ia_max_t06 IS NOT NULL THEN COALESCE(cm.cc_max_t06, 0) + COALESCE(im.ia_max_t06, 0) END AS module_06_max,
        CASE WHEN cm.cc_max_t07 IS NOT NULL OR im.ia_max_t07 IS NOT NULL THEN COALESCE(cm.cc_max_t07, 0) + COALESCE(im.ia_max_t07, 0) END AS module_07_max,
        CASE WHEN cm.cc_max_t08 IS NOT NULL OR im.ia_max_t08 IS NOT NULL THEN COALESCE(cm.cc_max_t08, 0) + COALESCE(im.ia_max_t08, 0) END AS module_08_max
    FROM base b
    LEFT JOIN cc_map cm ON cm.batch_name = b.batch_name
    LEFT JOIN ia_map im ON im.batch_name = b.batch_name
)
SELECT
    mc.*,
    CASE WHEN mc.module_01_total IS NOT NULL OR mc.module_02_total IS NOT NULL OR mc.module_03_total IS NOT NULL OR mc.module_04_total IS NOT NULL
        THEN RANK() OVER (
            PARTITION BY mc.batch_name, mc.course_code
            ORDER BY (
                COALESCE(mc.module_01_total, 0) +
                COALESCE(mc.module_02_total, 0) +
                COALESCE(mc.module_03_total, 0) +
                COALESCE(mc.module_04_total, 0)
            ) DESC
        )
    END AS rank_after_m4,
    CASE WHEN mc.module_01_total IS NOT NULL OR mc.module_02_total IS NOT NULL OR mc.module_03_total IS NOT NULL OR mc.module_04_total IS NOT NULL OR mc.module_05_total IS NOT NULL OR mc.module_06_total IS NOT NULL
        THEN RANK() OVER (
            PARTITION BY mc.batch_name, mc.course_code
            ORDER BY (
                COALESCE(mc.module_01_total, 0) +
                ...
                COALESCE(mc.module_03_total, 0) +
                COALESCE(mc.module_04_total, 0) +
                COALESCE(mc.module_05_total, 0) +
                COALESCE(mc.module_06_total, 0)
            ) DESC
        )
    END AS rank_after_m6,
    CASE WHEN mc.module_01_total IS NOT NULL OR mc.module_02_total IS NOT NULL OR mc.module_03_total IS NOT NULL OR mc.module_04_total IS NOT NULL OR mc.module_05_total IS NOT NULL OR mc.module_06_total IS NOT NULL OR mc.module_07_total IS NOT NULL OR mc.module_08_total IS NOT NULL
        THEN RANK() OVER (
            PARTITION BY mc.batch_name, mc.course_code
            ORDER BY (
                COALESCE(mc.module_01_total, 0) +
                ...
                COALESCE(mc.module_05_total, 0) +
                COALESCE(mc.module_06_total, 0) +
                COALESCE(mc.module_07_total, 0) +
                COALESCE(mc.module_08_total, 0)
            ) DESC
        )
    END AS rank_after_m8
    ,CASE
        WHEN EXISTS (
            SELECT 1
            FROM (VALUES
                (mc.cc_t01, 16), (mc.cc_t02, 16), (mc.cc_t03, 16), (mc.cc_t04, 16),
                (mc.cc_t05, 16), (mc.cc_t06, 16), (mc.cc_t07, 16), (mc.cc_t08, 16),
                (mc.ia_t01, 24), (mc.ia_t02, 24), (mc.ia_t03, 24), (mc.ia_t04, 24),
                (mc.ia_t05, 24), (mc.ia_t06, 24), (mc.ia_t07, 24), (mc.ia_t08, 24)
            ) AS v(score, min_required)
            WHERE v.score IS NOT NULL AND v.score < v.min_required
        ) THEN 'Fail'
        ELSE 'Pass'
      END AS pass_fail
FROM module_calc mc;
...
-- =============================================================================
-- VIEW 7: v_inference_failed_criteria
-- Purpose: One-row-per-student inference view with failed criteria reasons
-- Columns: prn, student_full_name, batch_name, course_code, centre_code,
--          cc_failed, ia_failed, ap_failed, sx_failed, ps_failed, gr_failed,
--          ta_failed, na_failed, in_failed, as_failed, pq_failed
-- =============================================================================

CREATE OR REPLACE VIEW v_inference_failed_criteria AS
WITH
cc_scores AS (
    SELECT
        sm.prn,
    ...
    LEFT JOIN scores_cc sc ON sc.prn = sm.prn
    CROSS JOIN LATERAL (
        SELECT array_agg(column_slot) AS slots, COUNT(*) AS n_configured
        FROM test_mapping WHERE batch_name = sm.batch_id AND category_code = 'CC' AND is_active = TRUE
    ) tm_agg
    WHERE sm.is_active = TRUE
),
...
SELECT
    cc.prn,
    cc.student_full_name,
    cc.batch_name,
    cc.course_code,
    cc.centre_code,
    CASE WHEN cc.cc_subtests_entered = 0 THEN 'Not Taken'
         ELSE array_to_string(
            array_remove(ARRAY[
                CASE WHEN cc.cc_s1 IS NOT NULL AND cc.cc_s1 = 0 THEN 'test_01 (0)'
                     WHEN cc.cc_s1 IS NOT NULL AND cc.cc_s1 < 16 THEN 'test_01 (' || ROUND(cc.cc_s1)::int || ' < 16)' END,
                CASE WHEN cc.cc_s2 IS NOT NULL AND cc.cc_s2 = 0 THEN 'test_02 (0)'
                     WHEN cc.cc_s2 IS NOT NULL AND cc.cc_s2 < 16 THEN 'test_02 (' || ROUND(cc.cc_s2)::int || ' < 16)' END,
                CASE WHEN cc.cc_s3 IS NOT NULL AND cc.cc_s3 = 0 THEN 'test_03 (0)'
                     WHEN cc.cc_s3 IS NOT NULL AND cc.cc_s3 < 16 THEN 'test_03 (' || ROUND(cc.cc_s3)::int || ' < 16)' END,
                CASE WHEN cc.cc_s4 IS NOT NULL AND cc.cc_s4 = 0 THEN 'test_04 (0)'
                     WHEN cc.cc_s4 IS NOT NULL AND cc.cc_s4 < 16 THEN 'test_04 (' || ROUND(cc.cc_s4)::int || ' < 16)' END,
                CASE WHEN cc.cc_s5 IS NOT NULL AND cc.cc_s5 = 0 THEN 'test_05 (0)'
                     WHEN cc.cc_s5 IS NOT NULL AND cc.cc_s5 < 16 THEN 'test_05 (' || ROUND(cc.cc_s5)::int || ' < 16)' END,
                CASE WHEN cc.cc_s6 IS NOT NULL AND cc.cc_s6 = 0 THEN 'test_06 (0)'
                     WHEN cc.cc_s6 IS NOT NULL AND cc.cc_s6 < 16 THEN 'test_06 (' || ROUND(cc.cc_s6)::int || ' < 16)' END,
                CASE WHEN cc.cc_s7 IS NOT NULL AND cc.cc_s7 = 0 THEN 'test_07 (0)'
                     WHEN cc.cc_s7 IS NOT NULL AND cc.cc_s7 < 16 THEN 'test_07 (' || ROUND(cc.cc_s7)::int || ' < 16)' END,
                CASE WHEN cc.cc_s8 IS NOT NULL AND cc.cc_s8 = 0 THEN 'test_08 (0)'
                     WHEN cc.cc_s8 IS NOT NULL AND cc.cc_s8 < 16 THEN 'test_08 (' || ROUND(cc.cc_s8)::int || ' < 16)' END,
                 CASE WHEN ROUND(cc.cc_raw_sum * 100.0 / NULLIF(cc.n_configured * 40.0, 0), 2) < 65 THEN 'Overall (' || to_char(ROUND(cc.cc_raw_sum * 100.0 / NULLIF(cc.n_configured * 40.0, 0), 2), 'FM999990.0') || '% < 65.0%)' END
            ], NULL), ', '
         )
    END AS cc_failed,
    CASE WHEN ia.ia_subtests_entered = 0 THEN 'Not Taken'
         ELSE array_to_string(
            array_remove(ARRAY[
                CASE WHEN ia.ia_s1 IS NOT NULL AND ia.ia_s1 = 0 THEN 'test_01 (0)'
                     WHEN ia.ia_s1 IS NOT NULL AND ia.ia_s1 < 24 THEN 'test_01 (' || ROUND(ia.ia_s1)::int || ' < 24)' END,
                CASE WHEN ia.ia_s2 IS NOT NULL AND ia.ia_s2 = 0 THEN 'test_02 (0)'
                     WHEN ia.ia_s2 IS NOT NULL AND ia.ia_s2 < 24 THEN 'test_02 (' || ROUND(ia.ia_s2)::int || ' < 24)' END,
                CASE WHEN ia.ia_s3 IS NOT NULL AND ia.ia_s3 = 0 THEN 'test_03 (0)'
                     WHEN ia.ia_s3 IS NOT NULL AND ia.ia_s3 < 24 THEN 'test_03 (' || ROUND(ia.ia_s3)::int || ' < 24)' END,
                CASE WHEN ia.ia_s4 IS NOT NULL AND ia.ia_s4 = 0 THEN 'test_04 (0)'
                     WHEN ia.ia_s4 IS NOT NULL AND ia.ia_s4 < 24 THEN 'test_04 (' || ROUND(ia.ia_s4)::int || ' < 24)' END,
                CASE WHEN ia.ia_s5 IS NOT NULL AND ia.ia_s5 = 0 THEN 'test_05 (0)'
                     WHEN ia.ia_s5 IS NOT NULL AND ia.ia_s5 < 24 THEN 'test_05 (' || ROUND(ia.ia_s5)::int || ' < 24)' END,
                CASE WHEN ia.ia_s6 IS NOT NULL AND ia.ia_s6 = 0 THEN 'test_06 (0)'
                     WHEN ia.ia_s6 IS NOT NULL AND ia.ia_s6 < 24 THEN 'test_06 (' || ROUND(ia.ia_s6)::int || ' < 24)' END,
                CASE WHEN ia.ia_s7 IS NOT NULL AND ia.ia_s7 = 0 THEN 'test_07 (0)'
                     WHEN ia.ia_s7 IS NOT NULL AND ia.ia_s7 < 24 THEN 'test_07 (' || ROUND(ia.ia_s7)::int || ' < 24)' END,
                CASE WHEN ia.ia_s8 IS NOT NULL AND ia.ia_s8 = 0 THEN 'test_08 (0)'
                     WHEN ia.ia_s8 IS NOT NULL AND ia.ia_s8 < 24 THEN 'test_08 (' || ROUND(ia.ia_s8)::int || ' < 24)' END,
                 CASE WHEN ROUND(ia.ia_raw_sum * 100.0 / NULLIF(ia.n_configured * 60.0, 0), 2) < 65 THEN 'Overall (' || to_char(ROUND(ia.ia_raw_sum * 100.0 / NULLIF(ia.n_configured * 60.0, 0), 2), 'FM999990.0') || '% < 65.0%)' END
            ], NULL), ', '
         )
    END AS ia_failed,
    CASE WHEN ap.ap_subtests_entered = 0 THEN 'Not Taken'
         ELSE array_to_string(
            array_remove(ARRAY[
                CASE WHEN ap.ap_s1 IS NOT NULL AND ap.ap_s1 = 0 THEN 'test_01 (0)'
                     WHEN ap.ap_s1 IS NOT NULL AND ap.ap_s1 < 48 THEN 'test_01 (' || ROUND(ap.ap_s1)::int || ' < 48)' END,
                CASE WHEN ap.ap_s2 IS NOT NULL AND ap.ap_s2 = 0 THEN 'test_02 (0)'
                     WHEN ap.ap_s2 IS NOT NULL AND ap.ap_s2 < 48 THEN 'test_02 (' || ROUND(ap.ap_s2)::int || ' < 48)' END,
                 CASE WHEN ap.ap_s3 IS NOT NULL AND ap.ap_s3 < 48 THEN 'test_03 (' || ROUND(ap.ap_s3)::int || ' < 48)' END,
                 CASE WHEN ap.ap_s4 IS NOT NULL AND ap.ap_s4 < 48 THEN 'test_04 (' || ROUND(ap.ap_s4)::int || ' < 48)' END,
                 CASE WHEN ap.ap_s5 IS NOT NULL AND ap.ap_s5 < 48 THEN 'test_05 (' || ROUND(ap.ap_s5)::int || ' < 48)' END
              ], NULL), ', '
            )
        END AS ap_failed,
        CASE WHEN sx.sx_subtests_entered = 0 THEN 'Not Taken'
            ELSE array_to_string(
              array_remove(ARRAY[
                 CASE WHEN sx.sx_s1 IS NOT NULL AND sx.sx_s1 = 0 THEN 'test_01 (0)'
                     WHEN sx.sx_s1 IS NOT NULL AND sx.sx_s1 < 30 THEN 'test_01 (' || ROUND(sx.sx_s1)::int || ' < 30)' END,
                 CASE WHEN sx.sx_s2 IS NOT NULL AND sx.sx_s2 = 0 THEN 'test_02 (0)'
                     WHEN sx.sx_s2 IS NOT NULL AND sx.sx_s2 < 30 THEN 'test_02 (' || ROUND(sx.sx_s2)::int || ' < 30)' END,
                 CASE WHEN sx.sx_s3 IS NOT NULL AND sx.sx_s3 < 30 THEN 'test_03 (' || ROUND(sx.sx_s3)::int || ' < 30)' END,
                 CASE WHEN sx.sx_s4 IS NOT NULL AND sx.sx_s4 < 30 THEN 'test_04 (' || ROUND(sx.sx_s4)::int || ' < 30)' END,
                 CASE WHEN sx.sx_s5 IS NOT NULL AND sx.sx_s5 < 30 THEN 'test_05 (' || ROUND(sx.sx_s5)::int || ' < 30)' END,
                 CASE WHEN ROUND(sx.sx_raw_sum * 100.0 / NULLIF(sx.n_configured * 60.0, 0), 2) < 50 THEN 'Overall (' || to_char(ROUND(sx.sx_raw_sum * 100.0 / NULLIF(sx.n_configured * 60.0, 0), 2), 'FM999990.0') || '% < 50.0%)' END
              ], NULL), ', '
            )
        END AS sx_failed,
        CASE WHEN ps.ps_subtests_entered = 0 THEN 'Not Taken'
            WHEN ROUND(ps.ps_raw_sum * 100.0 / NULLIF(ps.n_configured * 10.0, 0), 2) < 50 THEN 'Overall (' || to_char(ROUND(ps.ps_raw_sum * 100.0 / NULLIF(ps.n_configured * 10.0, 0), 2), 'FM999990.0') || '% < 50.0%)'
            ELSE 'Met' END AS ps_failed,
        CASE WHEN gr.gr_subtests_entered = 0 THEN 'Not Taken'
            WHEN ROUND(gr.gr_raw_sum * 100.0 / NULLIF(gr.n_configured * 10.0, 0), 2) < 50 THEN 'Overall (' || to_char(ROUND(gr.gr_raw_sum * 100.0 / NULLIF(gr.n_configured * 10.0, 0), 2), 'FM999990.0') || '% < 50.0%)'
            ELSE 'Met' END AS gr_failed,
                CASE WHEN ta_calc.ta_subtests_entered = 0 THEN 'Not Taken'
            ELSE array_to_string(
              array_remove(ARRAY[
                         CASE WHEN ta_calc.ta_s1 IS NOT NULL AND ta_calc.ta_s1 = 0 THEN 'test_01 (0)'
                             WHEN ta_calc.ta_s1 IS NOT NULL AND ta_calc.ta_s1 < 4 THEN 'test_01 (' || ROUND(ta_calc.ta_s1)::int || ' < 4)' END,
                         CASE WHEN ta_calc.ta_s2 IS NOT NULL AND ta_calc.ta_s2 = 0 THEN 'test_02 (0)'
                             WHEN ta_calc.ta_s2 IS NOT NULL AND ta_calc.ta_s2 < 4 THEN 'test_02 (' || ROUND(ta_calc.ta_s2)::int || ' < 4)' END,
                         CASE WHEN ta_calc.ta_s3 IS NOT NULL AND ta_calc.ta_s3 < 4 THEN 'test_03 (' || ROUND(ta_calc.ta_s3)::int || ' < 4)' END,
                         CASE WHEN ta_calc.ta_s4 IS NOT NULL AND ta_calc.ta_s4 < 4 THEN 'test_04 (' || ROUND(ta_calc.ta_s4)::int || ' < 4)' END,
                         CASE WHEN ta_calc.ta_s5 IS NOT NULL AND ta_calc.ta_s5 < 4 THEN 'test_05 (' || ROUND(ta_calc.ta_s5)::int || ' < 4)' END,
                         CASE WHEN ROUND(ta_calc.ta_raw_sum * 100.0 / NULLIF(ta_calc.n_configured * 10.0, 0), 2) < 40 THEN 'Overall (' || to_char(ROUND(ta_calc.ta_raw_sum * 100.0 / NULLIF(ta_calc.n_configured * 10.0, 0), 2), 'FM999990.0') || '% < 40.0%)' END
              ], NULL), ', '
            )
        END AS ta_failed,
        CASE WHEN na.na_subtests_entered = 0 THEN 'Not Taken'
            ELSE array_to_string(
              array_remove(ARRAY[
                 CASE WHEN na.na_s1 IS NOT NULL AND na.na_s1 = 0 THEN 'test_01 (0)'
                     WHEN na.na_s1 IS NOT NULL AND na.na_s1 < 4 THEN 'test_01 (' || ROUND(na.na_s1)::int || ' < 4)' END,
                 CASE WHEN na.na_s2 IS NOT NULL AND na.na_s2 = 0 THEN 'test_02 (0)'
                     WHEN na.na_s2 IS NOT NULL AND na.na_s2 < 4 THEN 'test_02 (' || ROUND(na.na_s2)::int || ' < 4)' END,
                 CASE WHEN na.na_s3 IS NOT NULL AND na.na_s3 < 4 THEN 'test_03 (' || ROUND(na.na_s3)::int || ' < 4)' END,
                 CASE WHEN na.na_s4 IS NOT NULL AND na.na_s4 < 4 THEN 'test_04 (' || ROUND(na.na_s4)::int || ' < 4)' END,
                 CASE WHEN na.na_s5 IS NOT NULL AND na.na_s5 < 4 THEN 'test_05 (' || ROUND(na.na_s5)::int || ' < 4)' END,
                 CASE WHEN ROUND(na.na_raw_sum * 100.0 / NULLIF(na.n_configured * 10.0, 0), 2) < 40 THEN 'Overall (' || to_char(ROUND(na.na_raw_sum * 100.0 / NULLIF(na.n_configured * 10.0, 0), 2), 'FM999990.0') || '% < 40.0%)' END
              ], NULL), ', '
            )
        END AS na_failed,
        CASE WHEN in_.in_subtests_entered = 0 THEN 'Not Taken'
            ELSE array_to_string(
              array_remove(ARRAY[
                 CASE WHEN in_.in_s1 IS NOT NULL AND in_.in_s1 = 0 THEN 'test_01 (0)'
                     WHEN in_.in_s1 IS NOT NULL AND in_.in_s1 < 8 THEN 'test_01 (' || ROUND(in_.in_s1)::int || ' < 8)' END,
                 CASE WHEN in_.in_s2 IS NOT NULL AND in_.in_s2 = 0 THEN 'test_02 (0)'
                     WHEN in_.in_s2 IS NOT NULL AND in_.in_s2 < 8 THEN 'test_02 (' || ROUND(in_.in_s2)::int || ' < 8)' END,
                 CASE WHEN in_.in_s3 IS NOT NULL AND in_.in_s3 < 8 THEN 'test_03 (' || ROUND(in_.in_s3)::int || ' < 8)' END,
                 CASE WHEN in_.in_s4 IS NOT NULL AND in_.in_s4 < 8 THEN 'test_04 (' || ROUND(in_.in_s4)::int || ' < 8)' END,
                 CASE WHEN in_.in_s5 IS NOT NULL AND in_.in_s5 < 8 THEN 'test_05 (' || ROUND(in_.in_s5)::int || ' < 8)' END
              ], NULL), ', '
            )
        END AS in_failed,
        CASE WHEN as_.as_subtests_entered = 0 THEN 'Not Taken'
            WHEN as_.as_subtests_entered < 25 THEN 'Completed (' || as_.as_subtests_entered || ' < 25)'
            ELSE 'Met' END AS as_failed,
        CASE WHEN pq.pq_subtests_entered = 0 THEN 'Not Taken'
            WHEN pq.pq_subtests_entered < 30 THEN 'Completed (' || pq.pq_subtests_entered || ' < 30)'
            ELSE 'Met' END AS pq_failed
    FROM cc_scores cc
    LEFT JOIN ia_scores ia ON ia.prn = cc.prn
    LEFT JOIN ap_scores ap ON ap.prn = cc.prn
    LEFT JOIN sx_scores sx ON sx.prn = cc.prn
    LEFT JOIN ps_scores ps ON ps.prn = cc.prn
    LEFT JOIN gr_scores gr ON gr.prn = cc.prn
    LEFT JOIN LATERAL (
        SELECT
            tm_agg.n_configured,
            tm_agg.raw_max,
        ...
        CROSS JOIN LATERAL (
            SELECT array_agg(column_slot) AS slots, COUNT(*) AS n_configured, SUM(max_marks) AS raw_max
            FROM test_mapping
            WHERE batch_name = cc.batch_name AND category_code = 'TA' AND is_active = TRUE
        ) tm_agg
        WHERE sc.prn = cc.prn
    ) ta_calc ON TRUE
    LEFT JOIN na_scores na ON na.prn = cc.prn
    LEFT JOIN in_scores in_ ON in_.prn = cc.prn
    LEFT JOIN as_scores as_ ON as_.prn = cc.prn
    LEFT JOIN pq_scores pq ON pq.prn = cc.prn;
